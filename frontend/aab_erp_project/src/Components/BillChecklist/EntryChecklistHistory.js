                    ...
                    <div className="bg-white rounded-lg shadow p-4 lg:w-[693px] h-[610px] space-y-2 overflow-auto">
                        {checklistUrls.length > 0 ? (
                            <table className="table-auto w-full border-collapse">
                                ...
                                <tbody>
                                    {checklistUrls.map((url, index) => {
                                        // For each URL, find the corresponding checklist number and date
                                        const checklistData = sortedExpenses.find(
                                            (expense) => expense.entryChecklistUrl === url
                                        );
                                        const checklistNumber = checklistData ? checklistData.checklistNumber : '';
                                        const timestamp = checklistData ? checklistData.timestamp : '';
                                        // Format the timestamp to DD/MM/YYYY
                                        const formattedDate = timestamp
                                            ? new Date(timestamp).toLocaleDateString('en-GB') // 'en-GB' gives DD/MM/YYYY format
                                            : '';
                                        const serialNumber = (checklistUrls.length - index).toString().padStart(2, '0');
                                        return (
                                            <tr key={index}>
                                                    ...
                                                    <div className='flex space-x-4'>
                                                        ...
                                                        <button
                                                            onClick={(e) => {
                                                                ...
                                                                if (checklistData && checklistData.checklistNumber) {
                                                                    const matchingEntries = sortedExpenses.filter(
                                                                        (expense) => expense.checklistNumber === checklistData.checklistNumber
                                                                    );

                                                                    setSelectedChecklist(matchingEntries);
                                                                    setSelectedDate(formattedDate);
                                                                    setSelectedChecklistNumber(checklistData.checklistNumber);
                                                                    setClickedChecklistNumber(checklistData.checklistNumber); // ✅ Set clicked one

                                                                    const url = matchingEntries[0]?.entryChecklistUrl;
                                                                    if (url) {
                                                            ...
                                                            style={{
                                                                color:
                                                                    clickedChecklistNumber === checklistData.checklistNumber
                                                                        ? '#ef6f47'
                                                                        : 'black',
                                                            }}
                                                        >
